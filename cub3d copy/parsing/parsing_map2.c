...
/*   Updated: 2022/07/18 13:33:18 by bleroy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

void	get_player2(t_game *g, int y, int x, char c)
{
	if (g->map[y][x - 1] != '0' && g->map[y][x - 1] != '1'
		&& g->map[y][x - 1] != 'D')
		error("Invalid map\n");
	if (g->map[y][x + 1] != '0' && g->map[y][x + 1] != '1'
		&& g->map[y][x + 1] != 'D')
		error("Invalid map\n");
	if (g->map[y - 1][x] != '0' && g->map[y - 1][x] != '1'
		&& g->map[y - 1][x] != 'D')
		error("Invalid map\n");
	if (g->map[y + 1][x] != '0' && g->map[y + 1][x] != '1'
		&& g->map[y + 1][x] != 'D')
		error("Invalid map\n");
	if (c == 'N')
		g->player.direction = 'N';
	if (c == 'E')
		g->player.direction = 'E';
	if (c == 'S')
		g->player.direction = 'S';
	if (c == 'W')
		g->player.direction = 'W';
	rotate_direction(g);
}
